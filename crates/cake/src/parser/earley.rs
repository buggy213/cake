...
fn predict(
    ...
    if nullable_nonterminals.contains(&nt) {
        let advanced_current_state = EarleyItem { dot: current_item.dot + 1, ..current_item };
...
pub fn earley_parse<T: LexemeSet>(input: &mut impl TokenStream<T>, grammar: &Grammar<T>) -> SPPF<T> {
...
// 2. DFS: 
fn construct_parse_trees<T: LexemeSet>(grammar: &Grammar<T>, earley_sets: Vec<Vec<EarleyItem>>, sppf: &mut SPPF<T>) {
    ...
    for (i, earley_set) in earley_sets.into_iter().enumerate() {
        for item in earley_set {
            let EarleyItem {back, dot, production} = item;
...
fn recursive_helper<T: LexemeSet>(
    ...
    sppf.insert(node, SPPFNode::Inner { derivations: Vec::new() });
    let mut trace = Vec::new();
    let (_, start, _) = node;
    dfs_helper(sppf, grammar, earley_sets, node, item.production, start, &mut trace);
}
    ...
    match next_symbol {
        Symbol::Terminal(_t) => {
            ...
            if sppf.contains_key(&key) {
                trace.push(key);
                dfs_helper(sppf, grammar, earley_sets, sppf_key, rule, position+1, trace);
...
pub fn make_graph_printers<T: LexemeSet>() 
    -> (Box<dyn Fn(&SPPFGraph<T>, (NodeIndex, &Option<SPPFNodeKey<T>>)) -> String>, Box<dyn Fn(&SPPFGraph<T>, u32) -> String>) {
