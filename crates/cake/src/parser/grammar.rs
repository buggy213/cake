...
impl<T: LexemeSet> Grammar<T> {
        ...
        let result = match symbol {
            ...
            EBNF::Identifier(x) => {
                let symbol = if let Some(name) = name_to_nonterminal.get(x.as_str()) {
                    ...
                    return Err(BNFError::Malformed(
                        "unable to find matching name or terminal",
                    ));
                };
            ...
            EBNF::Empty => {
                return Err(BNFError::Malformed(
                    "unexpected empty node (should only occur at top level productions)",
                ))
            }
                ...
                match (*low, *high) {
                    ...
                    (RangeUInt::Finite(low), RangeUInt::Finite(high)) => {
                        for i in low..=high {
                            if i == 0 {
                                ...
                                productions.push(Production::Nonempty(
                                    new_nt,
                                    vec![clause_symbol; i as usize],
                                ));
                            }
                        }
                    }
                    (RangeUInt::Finite(low), RangeUInt::Infinite) => {
                        if low == 0 {
                            ...
                            productions.push(Production::Nonempty(
                                new_nt,
                                vec![clause_symbol; low as usize],
                            ));
                        }
                        ...
                        productions.push(Production::Nonempty(
                            new_nt,
                            vec![new_nt_symbol, clause_symbol],
                        ))
                    }
    ...
    pub fn from_ebnf(ebnf: &[EBNF]) -> Result<Grammar<T>, BNFError> {
        ...
        Ok(Grammar {
            ...
            productions,
            goal_symbol,
            nonterminal_names: nonterminal_to_name
                .into_iter()
                .map(|x| x.to_string())
                .collect(),
        })
...
impl EBNF {
    ...
    fn parse_ebnf(input: &str) -> Result<Vec<EBNF>, BNFError> {
        ...
        for nonterminal in nonterminal_chunks {
            ...
            println!("'{}'", nonterminal);
            // super basic lexing
            let mut toks: VecDeque<&str> = nonterminal
                .split_whitespace()
                .map(|s| {
    ...
    fn parse_nonterminal(toks: &mut VecDeque<&str>) -> Result<EBNF, BNFError> {
        let symbol = toks.pop_front().ok_or(BNFError::UnexpectedEOF)?;
        ...
        Ok(EBNF::Nonterminal(symbol.to_string(), productions))
    }
    ...
    fn parse_production(toks: &mut VecDeque<&str>, top_level: bool) -> Result<EBNF, BNFError> {
        ...
        if factors.len() == 0 {
            if top_level {
                ...
                Err(BNFError::Malformed(
                    "empty terms not allowed unless part of a top-level production rule",
                ))
            }
        } else {
