...
mod consts {
    ...
    pub(super) fn entry_size(section: Section) -> u64 {
        match section {
            ...
            Section::Text | Section::Data | Section::Bss | Section::Strtab => 0,
        }
    }
}
...
impl Elf {
    ...
    pub(crate) fn undefined_symbol(
        ...
        let name = self.string_table.add_string(name);
        self.symbol_table
            .add_symbol(name, binding, symbol_type, None, 0usize, 0usize)
    }
    ...
    pub(crate) fn reloc_text(
        ...
        self.rela_text
            .add_relocation(offset, symtab_idx, reloc_type, addend);
    }
    ...
    pub(crate) fn write(&self, writer: &mut impl std::io::Write) -> std::io::Result<()> {
        ...
        let ident = [
            // magic
            0x7f, b'E', b'L', b'F', // 64-bit
            0x02, // little-endian
            0x01, // version
            0x01, // sysv abi
            0x00, // abi version 0
            0x00, // padding
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // size of ident
            0x10,
        ];
        ...
        let section_header_offset = HEADER_SIZE as usize
        ...
        self.rela_text
            .write(writer, self.symbol_table.local_symbol_count())?;
        ...
        fn write_section_header(
            ...
            let section_flags = match section {
                ...
                Section::RelaText | Section::RelaData | Section::Symtab | Section::Strtab => {
        ...
        write_section_header(
            writer,
            self.section_name_offsets[5],
            Section::Symtab,
            symbol_table_offset,
            symbol_table_size,
            self.symbol_table.local_symbol_count(),
        )?;
        write_section_header(
            writer,
            self.section_name_offsets[6],
            Section::Strtab,
            string_table_offset,
            string_table_size,
            0,
    ...
    fn add_symbol(
        ...
        let idx = match binding {
            ElfSymbolBinding::Local => {
                SymbolTableIndex::LocalSymbol(self.local_symbols.len() as u32)
            }
            ElfSymbolBinding::Global | ElfSymbolBinding::Weak => {
                SymbolTableIndex::NonlocalSymbol(self.nonlocal_symbols.len() as u32)
            }
        };
...
mod test {
    ...
    fn test_basic() {
        ...
        let main_addr = elf.add_text(&[
            ...
            0x8b, 0x05, 0x00, 0x00, 0x00, 0x00, // ret
            0xc3,
        ]);
    ...
    fn test_reloc_data_and_link() {
        ...
        elf0.add_text(&[
            // xor eax, eax
            0x31, 0xc0, // add rsp, 0x8
            0x48, 0x83, 0xc4, 0x08, // ret
            0xc3,
        ]);
        ...
        let main_addr_sym =
            elf0.undefined_symbol("main_addr", ElfSymbolBinding::Global, ElfSymbolType::Object);

        let printf_sym =
            elf0.undefined_symbol("printf", ElfSymbolBinding::Global, ElfSymbolType::Func);
        ...
        let main_sym = elf1.undefined_symbol("main", ElfSymbolBinding::Global, ElfSymbolType::Func);
