...
impl Elf64SectionHeader {
    fn parse_section_header(data: &[u8]) -> Result<Self, ElfError> {
        ...
        let flags: ElfSectionFlags = ElfSectionFlags::from_bits(flags).ok_or(ElfError::ElfSectionFlagsError(flags))?;
    ...
    pub fn new(buffer: &'buffer [u8]) -> Result<Self, ElfError> {
        ...
        if elf_header.section_header_table_offset != 0 {
            for k in 0..(elf_header.section_header_count as usize){
                let index = elf_header.program_header_table_offset as usize + k * (elf_header.section_header_size as usize);
                let section_header_bytes = &buffer[index..];
                let section_header = Elf64SectionHeader::parse_section_header(section_header_bytes)?;
